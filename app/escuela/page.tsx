  ...
  return (
    <div className="relative w-full h-56 mb-8 rounded-[2rem] overflow-hidden group/item bg-stone-200/20 border border-stone-100 dark:border-stone-900">
      ...
      <div className="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover/item:opacity-100 transition-opacity z-10">
        <button
          onClick={prev}
          className={`p-2 rounded-full backdrop-blur-xl border ${darkMode ? "bg-black/40 border-white/10 text-white" : "bg-white/40 border-black/5 text-stone-800"}`}
        >
          <ChevronLeft size={16} />
        </button>
        <button
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
        {fotos.map((_, i) => (
          <div
            key={i}
            className={`h-1 rounded-full transition-all duration-500 ${current === i ? "w-4 bg-white" : "w-1 bg-white/30"}`}
          />
        ))}
      </div>
    </div>
        ...
        <div className="w-full max-w-4xl mx-auto p-6 md:p-12 space-y-32">
          ...
          <header className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[9px] uppercase tracking-[0.6em] text-stone-400 font-bold block">
                    Generación de Relevo
                  </span>
                </div>
                <h1
                  className={`text-5xl md:text-7xl font-extralight tracking-tighter leading-none ${darkMode ? "text-stone-100" : "text-stone-900"}`}
                >
                  Sembrando en <br />
                ...
                <p className="max-w-xs text-[13px] leading-relaxed opacity-60">
                  Acompañamos a cada niño en el viaje de descubrir su propósito
                  divino con amor y creatividad.
                </p>
              </div>
            ...
            <div className="relative group max-w-sm mx-auto md:ml-auto">
              ...
              <div
                className={`relative aspect-square rounded-[3rem] ${darkMode ? "shadow-none" : "shadow-[0_32px_64px_-15px_rgba(0,0,0,0.1)]"} transition-all duration-700`}
              >
                {/* Contenedor del video con recorte perfecto */}
                <div
                  className={`w-full h-full overflow-hidden rounded-[3rem] border isolate ${darkMode ? "border-stone-800 bg-stone-900" : "border-white bg-white"}`}
                >
                  <video
                    autoPlay
                    loop
                  ...
                  <div
                    className={`absolute inset-0 pointer-events-none ${darkMode ? "bg-black/20" : "bg-stone-500/5"}`}
                  />
                </div>

                {/* Elemento decorativo flotante (opcional, para romper la simetría) */}
                <div
                  className={`absolute -bottom-4 -left-4 w-20 h-20 border-l border-b rounded-bl-[2rem] opacity-20 ${darkMode ? "border-stone-100" : "border-stone-900"}`}
                />
              </div>
            </div>
          </header>
            ...
            {[
              {
                ...
                t: "Seguridad",
                d: "Protocolos estrictos en cada salón.",
              },
              {
                icon: <BookOpen size={20} />,
                t: "Creatividad",
                d: "La Biblia contada con asombro.",
              },
              {
                icon: <Heart size={20} />,
                t: "Comunidad",
                d: "Amistades con valores eternos.",
              },
            ].map((item, i) => (
              <div key={i} className="text-center space-y-4">
                ...
                <h5 className="text-[10px] font-black uppercase tracking-widest text-stone-400">
                  {item.t}
                </h5>
          ...
          {/* --- INFO FINAL --- */}
          <section
            className={`p-12 md:p-20 rounded-[4rem] text-center space-y-8 border ${darkMode ? "bg-[#0f0f0f] border-stone-800" : "bg-white border-stone-100 shadow-2xl"}`}
          >
            <div className="space-y-4">
              <Sun
          ...
          <footer className="py-20 text-center border-t border-stone-100 dark:border-stone-900 mt-20 px-6">
            <div className="max-w-2xl mx-auto space-y-8">
              {/* Cita Bíblica */}
              <div className="space-y-4">
                <p
                  className={`text-sm md:text-base uppercase tracking-[0.3em] font-medium leading-relaxed italic ${
                    darkMode ? "text-stone-300" : "text-stone-600"
                  }`}
                >
                  "Dejad que los niños vengan a mí, porque de ellos es el reino
                  de los cielos."
                </p>
                <span
              ...
              <div className="flex justify-center items-center gap-3 opacity-20">
                <div
                  className={`h-px w-8 ${darkMode ? "bg-stone-100" : "bg-stone-900"}`}
                />
                <div
